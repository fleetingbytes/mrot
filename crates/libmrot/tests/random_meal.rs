...
async fn get_random_meal(world: &mut World) -> Result<()> {
    ...
    world.storage_random_result = Some(result);
    Ok(())
}

#[then(regex = r"^I get no meal record")]
async fn get_no_meal_record(world: &mut World) -> Result<()> {
    let actual_record = world.storage_random_result.as_ref().ok_or(Error::UndefinedValue("storage_random_result".to_string()))?.as_ref().map_err(|e| Error::UnexpectedErrResult(format!("{:?}", e)))?;
...
async fn verify_one_random_meal(world: &mut World, meals: Meals) -> Result<()> {
    let actual_record = world.storage_random_result.as_ref().ok_or(Error::UndefinedValue("storage_random_result".to_string()))?.as_ref().map_err(|e| Error::UnexpectedErrResult(format!("{:?}", e)))?.clone().ok_or(Error::UndefinedValue("the Option in storage_random_result was None".to_string()))?;
